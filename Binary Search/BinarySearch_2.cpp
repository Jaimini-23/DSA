...
vector<int> findPeakGrid(vector<vector<int>> &arr)  {            // peak element in 2D matrix
    // TC: O(n logm) and SC: O(1)
    int n = arr.size();
    int m = arr[0].size();
    int low = 0, high = m - 1;
    while(low <= high) {
...
int median_in_2D_array(vector<vector<int>> &matrix, int m, int n) {         // row wise sorted 2D array
    int low = INT_MAX;
    int high = INT_MIN;
    ...
    while(low <= high) {
        int mid = low + (high - low) / 2;
        int smallEqual = countSmallerEqual(matrix, n, m, mid);
        if(smallEqual <= req)
            low = mid + 1;
        else
            high = mid - 1;
